...
export default {
  ...
  ready: function() {
    const self = this
    $.getJSON('../../../static/data/comment.json', function(result) {
  ...
  methods: {
    ...
    replyFloor: function (index) {
      const id = this.commentList[index].id
      const floor = this.commentList[index].floor
      console.log('正在回复' + floor + '楼')
      console.log(id)
      this.replyTo = floor
    }
  }
...
.comment_list .list_head{

}


.comment_list .list_head img{
    width: 60px;
    height: 60px;
    border-radius: 50%;
...
.comment_list .list_name{
    color: #31a831;
}


.comment_list .list_info .info_right{
	float: right;
	font-size: 12px;
}
	
.comment_list .list_info time{
	padding-right: 10px;
}

.comment_list .list_info .floor{
    color: #31a831;
}


.comment_list .list_say{
	display:block;
	padding-left: 70px;
	padding-bottom: 10px;
	line-height: 25px;
...
.comment_list .list_reply .list_say{
	font-size:90%;
}

.comment_list .list_reply .list_head img{
	width: 40px;
    height: 40px;
}

.comment_list .list_tool{
	float: right;
}

.comment_list .list_tool span{
	padding-left:7px;
}
</style>
